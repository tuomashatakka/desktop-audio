...
export class ElectronBridge implements Bridge {
  ...
  loadLibrary(): Promise<readonly TrackDTO[]> {
    return this._ipc.loadLibrary() as Promise<readonly TrackDTO[]>
  }

  onLibraryBatch(cb: (t: TrackDTO[]) => void): () => void {
    return this._ipc.onLibraryBatch(cb)
  }
  ...
  selectDirectory(): Promise<string | null> {
    return this._ipc.selectDirectory()
  }

  getMusicDir(): Promise<string | null> {
    return this._ipc.getMusicDir() as Promise<string | null>
  }

  readFile(path: string): Promise<ArrayBuffer> {
    return this._ipc.readFile(path)
  }
